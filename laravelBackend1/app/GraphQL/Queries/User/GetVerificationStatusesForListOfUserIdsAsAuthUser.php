...
class GetVerificationStatusesForListOfUserIdsAsAuthUser extends Query {
    protected $redisClient;
    protected $userAuthService;

    protected $attributes = [
        'name' => 'getVerificationStatusesForListOfUserIdsAsAuthUser',
    ];
    ...
    public function type(): Type {
        return Type::listOf(Type::boolean());
    }
    ...
    public function resolve($args, Request $request) {
        ...
        $usersAndTheirVerificationStatuses = [];
        ...
        try {
            ...
            for ($i=0; $i<count($redisResults); $i++) {
                $userId = $userIds[$i];
                $isVerified = $redisResults[$i];
                $usersAndTheirVerificationStatuses[$userId] = $isVerified;
            }

            foreach($originalUserIds as $originalUserId) {
                $output[] =  $usersAndTheirVerificationStatuses[$originalUserId] ?? null;
            }
        ...
        try {
            $publicUsersAndTheirVerificationStatuses = PublicUser::whereIn('id', $userIds)
                ->pluck('isVerified', 'id')
                ->toArray();
            
            foreach (array_keys($publicUsersAndTheirVerificationStatuses) as $publicUserId) {
                $usersAndTheirVerificationStatuses[$publicUserId] = $publicUsersAndTheirVerificationStatuses[$publicUserId];
            }

            $privateUserIds = array_filter($userIds, function($userId) use ($usersAndTheirVerificationStatuses) {
                return !array_key_exists($usersAndTheirVerificationStatuses, $userId);
            });

            if (count($privateUserIds) > 0) {
                $privateUsersAndTheirVerificationStatuses = PrivateUser::whereIn('id', $privateUserIds)
                    ->pluck('isVerified', 'id')
                    ->toArray();
            
                foreach (array_keys($privateUsersAndTheirVerificationStatuses) as $privateUserId) {
                    $usersAndTheirVerificationStatuses[$privateUserId] = $privateUsersAndTheirVerificationStatuses[$privateUserId];
                }
            }
